module ApplicationHelper
  ...
  def build_submit_button(title) 
    raw("<button type='submit' class='button positive'> #{image_tag('icons/positive.png')} #{title} </button>")
  end
  
  def build_link_button(title, path)
    raw("<p class='buttons'><a href='#{path}' title='#{title}' class='button positive'>#{image_tag('icons/positive.png')} #{title}</a></p>")
  end
